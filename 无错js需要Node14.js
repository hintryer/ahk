 ...
 */
function buildIndentationChars(depth, options) {
 ...
 */
function buildIndentedString(indentationChars, formattedLine, preserveIndentOnEmptyString) {
 ...
 */
function buildIndentedLine(lineIndex, lastLineIndex, formattedLine, depth, options) {
    // 1. 生成当前深度的缩进字符
    const indentationChars = buildIndentationChars(depth, options);
    // 2. 生成带缩进的行文本（无换行符）
    let indentedLine = buildIndentedString(indentationChars, formattedLine, options.preserveIndent);
 ...
 */
function hasMoreCloseParens(line) {
    if (!line.includes(')'))
        return false; // 无右括号直接返回 false
 ...
 */
function hasMoreOpenParens(line) {
    if (!line.includes('('))
        return false; // 无左括号直接返回 false
 ...
 */
function nextLineIsOneCommandCode(text) {
    // 需触发下一行缩进的控制流关键字列表
 ...
 * @returns 清理空格后的行文本
 */
function trimExtraSpaces(line, trimExtraSpaces) {
 ...
 * @returns 未匹配的目标大括号数量
 */
function braceNumber(line, braceChar) {
 ...
 */
function alignTextAssignOperator(text) {
    /**
     * 步骤1：计算所有赋值语句中「第一个等号」的最右侧位置
     * - 先通过 `normalizeLineAssignOperator` 标准化每行格式（清理注释、统一空格）
     * - 再获取标准化后每行等号的索引，取最大值作为「目标对齐位置」
     */
    const maxPosition = Math.max(...text.map((line) => normalizeLineAssignOperator(line).indexOf('=')));
    /**
     * 步骤2：按最右侧位置对齐所有等号
     * - 调用 `alignLineAssignOperator` 为每行补充空格，使等号统一移动到 `maxPosition`
     */
    const alignedText = text.map((line) => alignLineAssignOperator(line, maxPosition));
    return alignedText;
}
/**
 * 【赋值语句标准化】
 ...
 */
function normalizeLineAssignOperator(original) {
    return (original
 ...
 */
function alignLineAssignOperator(original, targetPosition) {
 ...
 * @return 注释对齐后的文档字符串
 */
function alignSingleLineComments(stringToFormat, options) {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
        ...
        if (emptyLine) {
            ...
            lines[i] = buildIndentedString(indentationChars, line.trim(), // 清理原缩进，按计算的缩进重新对齐
 ...
 */
function calculateDepth(text, options) {
    // 匹配行首的所有空白字符（空格或制表符）
 ...
 * @return 完全格式化后的文档字符串
 */
const internalFormat = (stringToFormat, options) => {
    ...
    let assignmentBlock = []; // 存储待对齐的赋值语句块（多行赋值）
    // ==============================
    // 4. 续行相关状态变量
    // ==============================
    ...
    const continuationSection = /^(((and|or|not)\b)|[\^!~?:&<>=.,|]|\+(?!\+)|-(?!-)|\/(?!\*)|\*(?!\/))/;
    const label = /^[^\s\t,`]+(?<!:):$/; // 标签匹配（如 Label:）
    const hotkey = /^.+::$/; // 热键/热字符串匹配（无代码，如 F1::）
    const hotkeySingleLine = /^.+::/; // 单行热键匹配（含代码，如 F1::Run Notepad）
    const sharpDirective = '#(ifwinactive|ifwinnotactive|ifwinexist|ifwinnotexist|if)'; // #If 指令匹配
    ...
    lines.forEach((originalLine, lineIndex) => {
        ...
        if (emptyLine) {
            if (alignAssignment && comment.match(ahkAlignAssignmentOff)) {
                ...
                assignmentBlock.forEach((alignedFormattedLine, index) => {
                    formattedString += buildIndentedLine(lineIndex - assignmentBlock.length + index + 1, lines.length, alignedFormattedLine, depth, options);
                });
                assignmentBlock = [];
            }
        ...
        if (blockComment) {
            if (!formatBlockComment) {
                ...
                formattedString += buildIndentedLine(lineIndex, lines.length, blockCommentLine.trimEnd(), depth, options);
            }
        ...
        if (continuationSectionTextFormat) {
            formattedString += buildIndentedLine(lineIndex, lines.length, originalLine.trim(), depth, options);
            return;
        }
        // 表达式/对象续行处理
        if (purifiedLine.match(continuationSection) &&
            !purifiedLine.match(/::/)) {
        ...
        // 处理左大括号（进入代码块）
        if (openBraceNum) {
            if ((oneCommandCode || deferredOneCommandCode) &&
        ...
        if ((ifDepth.last() > -1 || focDepth.last() > -1) &&
        ...
        if (purifiedLine.match(/^(return|exit|exitapp)\b/) &&
            tagDepth === depth) {
            tagDepth = 0;
            depth--;
        }
...
const options2 = {
    ...
    indentCodeAfterLabel: 1,  // 标签后代码的缩进量
    indentCodeAfterIfDirective: 1,  // if 指令后代码的缩进量
    trimExtraSpaces: true   // 是否清理多余空格（用户配置）
};
const multiLineText = `if    (true) {return {}} else

dfaf()`;
text=internalFormat(multiLineText,options2)
console.log(text);
