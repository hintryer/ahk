...
import { commentRegExp } from '../common/constants'; // 导入注释匹配正则（如匹配单行注释";"）
import { FormatOptions } from './formattingProvider.types'; // 导入格式化配置选项类型定义
import { isDeepStrictEqual } from 'util'; // 导入深度严格相等校验工具（用于调试时校验数据）
...
export const documentToString = (document: {
    ...
    new Array(document.lineCount) // 创建与行数相等的空数组
        .fill(0) // 填充占位值（仅用于遍历索引）
        .map((_v, i) => document.lineAt(i).text) // 遍历每一行，获取行文本
        .join('\n'); // 用\n连接所有行，统一换行符
...
export function hasMoreCloseParens(line: string): boolean {
    ...
    const closeCount = line.match(/\)/g).length; // 右括号数量（已确认有右括号，无需判空）
...
export const internalFormat = (
    ...
    const ifDepth = new FlowOfControlNestDepth(); // if-else 嵌套深度管理器（单独管理，避免与其他控制流混淆）
    const focDepth = new FlowOfControlNestDepth(); // 通用控制流（loop/while/for 等）深度管理器
